...
const start = ({ importmap: importMapSource, lang = 'en' }: StartOptions) => {
  ...
  router.get('/:slug+.js', async (context, next) => {
    ...
    let file = existsSync(join(Deno.cwd(), 'src', jsx)) ? jsx : existsSync(join(Deno.cwd(), 'src', tsx)) ? tsx : false
    if (!file) return await next()
    try {
      const source = await Deno.readTextFile(join(Deno.cwd(), 'src', ...file.split('/')))
